      ...
      <div className="mx-auto w-full max-w-[1440px] px-4 py-16 sm:px-8 lg:px-12">
        ...
        <div className="mt-12 flex items-start gap-3 border border-blood/40 bg-blood/5 p-5">
          ...
          <p className="text-[11px] leading-relaxed text-cream/55 sm:text-xs">
            {isZh ? <><span className="font-bold text-blood">风险提示：</span>本页面内容不构成财务建议。代币机制仅作为协议设计介绍，销毁、费用和奖励流不保证收益、升值、交易所上架或成功。参与链上协议存在风险，包括本金全部损失。请仅通过官方渠道核验合约地址和付款说明。</> : <><span className="font-bold text-blood">RISK DISCLOSURE:</span> Nothing on this page is financial advice. Token mechanics are described as protocol design — burns, fees and reward flows are not guarantees of profit, appreciation, exchange listing or success. Participation in on-chain protocols carries risk, including total loss. Verify contract addresses and payment instructions only through official channels.</>}
          </p>
        </div>
        ...
        <div className="mt-8 flex flex-col items-center justify-between gap-4 border-t border-gold/15 pt-8 text-[11px] tracking-widest text-cream/40 uppercase sm:flex-row">
          <p>{isZh ? "© 2026 FOMO Life • 无 CEO，只有代码" : "© 2026 FOMO Life • No CEO, only code"}</p>
          <div className="flex gap-6">
            <a href="#vault" className="hover:text-gold">{isZh ? "金库" : "Vault"}</a>
            <a href="#tokenomics" className="hover:text-gold">{isZh ? "代币经济" : "Tokenomics"}</a>
            <a href="#ido" className="hover:text-gold">IDO</a>
            <a href="#faq" className="hover:text-gold">{isZh ? "常见问题" : "FAQ"}</a>
          </div>
        </div>
      </div>
    </footer>
